...
| `KEYCLOAK_ADMIN_PASSWORD` | Required Keycloak bootstrap administrator password for Docker Compose instance rendering. |
| `KEYCLOAK_DEV_PASSWORD` | Required password for the development realm user generated by the instance renderer. |
| `BEDROCK_ENCRYPT_KEY` | Base64 AES-256-GCM key used to encrypt persisted target passwords and API responses. |
| `BEDROCK_STATE_DB_URL` | JDBC PostgreSQL URL for persistent API state. |
| `BEDROCK_STATE_DB_USER` | Dedicated PostgreSQL state-store role. |
| `BEDROCK_STATE_DB_PASSWORD` | Password for the dedicated state-store role. |
| `BEDROCK_STATE_DB_SCHEMA` | Dedicated schema for API state. Defaults to `schema_migrator`. |
| `BEDROCK_STATE_DB_POOL_SIZE` | Maximum state-store connection pool size. Defaults to `10`. |
...
Schema Migrator API state and its active Keycloak configuration snapshot live in
the normalized `schema_migrator` PostgreSQL schema. To install or query the report view:
