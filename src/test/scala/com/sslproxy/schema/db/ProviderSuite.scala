...
class ProviderSuite extends FunSuite:
  ...
  test("rejects JDBC postgres URLs without a host") {
    assertEquals(
      PostgresProvider.normalize("jdbc:postgresql:///sync").left.toOption,
      Some("invalid Postgres URL: host is required")
    )
  }
  ...
  test("postgres skipped apply log preserves existing applied timestamp") {
    val tx = Transactor.fromDriverManager[IO]("org.h2.Driver", h2Url(), "", "", None)
    val obj = SchemaObject(
      kind = "tables",
      objectName = "accounts",
    ...
    val program =
      for
        ...
        _ <-
          sql"""
          insert into schema_control.schema_objects (
            kind, object_name, source_file, content_sha256, apply_status, applied_at
          ) values (${obj.kind}, ${obj.objectName}, ${obj.sourceFile}, 'same-sha', 'applied', now())
          """.update.run
        _ <- ApplyLog.postgres("provider-suite").recordSkipped(obj, Some("same-sha"))
        status <-
          sql"""
          select apply_status, applied_at is not null
            from schema_control.schema_objects
           where kind = ${obj.kind} and object_name = ${obj.objectName}
          """.query[(String, Boolean)].unique
      yield status

    assertEquals(program.transact(tx).unsafeRunSync(), ("skipped", true))
  }

  test("postgres apply log recovers cleanly when replay succeeds after a failed attempt") {
    val tx = Transactor.fromDriverManager[IO]("org.h2.Driver", h2Url(), "", "", None)
    val obj = SchemaObject(
      kind = "tables",
      objectName = "accounts",
    ...
    val program =
      for
        ...
        _ <-
          ...
          insert into schema_control.schema_objects (
            kind, object_name, source_file, content_sha256, apply_status
          ) values (${obj.kind}, ${obj.objectName}, ${obj.sourceFile}, ${obj.sha256}, 'pending')
          """.update.run
        _ <- ApplyLog.postgres("provider-suite").recordFailed(obj, Some("old-sha"), 7, "boom")
        _ <- ApplyLog.postgres("provider-suite").recordApplied(obj, Some("old-sha"), 11)
        status <-
          sql"""
          select apply_status, content_sha256, last_error is null
            from schema_control.schema_objects
           where kind = ${obj.kind} and object_name = ${obj.objectName}
          """.query[(String, String, Boolean)].unique
        actions <-
          sql"""
          select action
            from schema_control.schema_apply_log
           where kind = ${obj.kind} and object_name = ${obj.objectName}
           order by applied_at, action
          """.query[String].to[List]
      yield (status, actions.sorted)

    assertEquals(program.transact(tx).unsafeRunSync(), (("applied", "new-sha", true), List("applied", "failed")))
  }
  ...
  test("oracle lock conflict query reports current holder without age-based cleanup") {
    ...
    val normalized = sql.toLowerCase
    assert(normalized.contains("locked_by"))
    assert(normalized.contains("locked_at_char"))
    assert(normalized.contains("where lock_name = 'schema_migrate'"))
    assert(!normalized.contains("systimestamp"))
    assert(!normalized.contains("sysdate"))
    assert(!normalized.contains("interval"))
    assert(!normalized.contains("delete from schema_control.migration_locks"))
  }
  ...
  test("oracle bootstrap blocks are idempotent schema_control DDL") {
    ...
    val lowerBlocks = blocks.map(_.toLowerCase)
    ...
    assert(joined.contains("alter table schema_control.migration_locks"))
    ...
    val schemaObjectsIndex = lowerBlocks.indexWhere(_.contains("create table schema_control.schema_objects"))
    val rollbackAlterIndex = lowerBlocks.indexWhere(_.contains("alter table schema_control.schema_objects add"))
    val migrationLocksIndex = lowerBlocks.indexWhere(_.contains("create table schema_control.migration_locks"))
    val migrationLocksPkIndex = lowerBlocks.indexWhere(_.contains("alter table schema_control.migration_locks"))
    val readyIndex = lowerBlocks.indexWhere(_.contains("create or replace view schema_control.schema_ready"))

    assert(schemaObjectsIndex >= 0)
    assert(rollbackAlterIndex > schemaObjectsIndex)
    assert(migrationLocksPkIndex > migrationLocksIndex)
    assertEquals(readyIndex, blocks.length - 1)
  }
