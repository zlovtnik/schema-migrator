...
class JwtMiddlewareSuite extends FunSuite:
  test("middleware rejects missing tokens and accepts signed bearer token for protected routes") {
    ...
    val bearer = routes
      ...
      .unsafeRunSync()
    val query = routes.run(Request[IO](Method.GET, Uri.unsafeFromString(s"/protected?access_token=$token"))).unsafeRunSync()
  ...
  test("middleware limits query token fallback to run stream routes") {
    val config = ServerConfig(
      host = "127.0.0.1",
      port = 8080,
      corsOrigins = Set("http://localhost:5173"),
    ...
    val routes = JwtMiddleware(config)(HttpRoutes.of[IO] {
      case GET -> Root / "api" / "runs" / _ / "stream" => Ok("ok")
    }).orNotFound
    val token = JwtTokens.create(config.jwtSecret, "tester").map(_._1).unsafeRunSync()

    val stream = routes.run(Request[IO](Method.GET, Uri.unsafeFromString(s"/api/runs/run-1/stream?access_token=$token"))).unsafeRunSync()

    assertEquals(stream.status, Status.Ok)
  }
